...
const lockPallet = () => {
  const confirmation = confirm("Are you sure you want to complete this pallet?");
...
const unlockPallet = () => {
  ...
  if (managerCode === "1234") {
      if (loadID && palletID && loads[loadID] && loads[loadID][palletID]) {
          ...
          alert(`Pallet ${palletID} is now unlocked. You can now assign a location.`);
      } else {
          alert("Invalid LoadID or PalletID.");
      }
  } else {
...
const addProduct = () => {
  ...
  if (loadID && palletID && loads[loadID] && loads[loadID][palletID]) {
      ...
      if (loads[loadID][palletID].locked) {
          alert(`Pallet ${palletID} is locked and cannot have more products added.`);
          return;
      }
...
// Add a delay to handle quick scans:
let barcodeInputTimeout;
document.getElementById("productID").addEventListener("input", () => {
  clearTimeout(barcodeInputTimeout);
  barcodeInputTimeout = setTimeout(() => {
    // Add a small delay to handle quick barcode scans
    addProduct();
